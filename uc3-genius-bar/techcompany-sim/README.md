...
Hackathon data simulator for **UC3 — Retail Genius Bar Triage and Escalation Agent**.
...
pip install -e uc3-genius-bar/techcompany-sim
```
...
|----------|-------------|
| `/appointment-queue/events` | Incoming Genius Bar appointments |
| `/device-diagnostics/events` | Device telemetry snapshots by serial number |
| `/repair-history/events` | Prior service events by serial number |
| `/parts-scheduling/events` | Parts inventory and technician availability |

**Seeded Anomaly:** Handset Pro 15 units from lot J7Q arrive with recurring haptic-engine failures (previously repaired) — a pattern that warrants depot escalation and an engineering flag.

---

## API Endpoints

Once running, visit `http://localhost:8003/docs` for the interactive Swagger UI.
...
```bash
sim-generate-uc3 --output ./seed-data/
```
...
```bash
docker build \
  -t <ecr-repo>/techcompany-sim-uc3:latest \
  -f uc3-genius-bar/techcompany-sim/Dockerfile \
  .
docker push <ecr-repo>/techcompany-sim-uc3:latest
```
...
```
http://techcompany-sim-uc3:8003/<source>/events
```
...
```bash
sim-uc3 --seed 1337 --anomaly-delay 60
```
