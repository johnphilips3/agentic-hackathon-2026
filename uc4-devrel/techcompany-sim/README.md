...
sim-uc4 --port 8004 --anomaly-delay 60

# Single window — anomaly fires after 60s, lasts 120s, then stops
sim-uc4 --port 8004 --anomaly-delay 60 --anomaly-duration 120

# Cycling — anomaly fires for 60s, recovers for 30s, repeats
sim-uc4 --port 8004 --anomaly-delay 30 --anomaly-duration 60 --anomaly-cycle-interval 30
```
...
```bash
docker build \
  -t <ecr-repo>/techcompany-sim-uc4:latest \
  -f uc4-devrel/techcompany-sim/Dockerfile \
  .
