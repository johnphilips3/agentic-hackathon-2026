...
Hackathon data simulator for **UC5 — Hardware Failure Pattern Detection and Field Action Agent**.
...
pip install -e uc5-hardware-failure/techcompany-sim
```
...
```bash
sim-uc5 --port 8005 --anomaly-delay 60 --anomaly-intensity obvious
```
...
sim-uc5 --port 8005 --anomaly-delay 60

# Single window — anomaly fires after 60s, lasts 120s, then stops
sim-uc5 --port 8005 --anomaly-delay 60 --anomaly-duration 120

# Cycling — anomaly fires for 60s, recovers for 30s, repeats
sim-uc5 --port 8005 --anomaly-delay 30 --anomaly-duration 60 --anomaly-cycle-interval 30
```
...
|----------|-------------|
| `/applecare-cases/events` | Support case records with symptom codes |
| `/repair-depot/events` | Structured depot repair reports |
| `/warranty-claims/events` | Warranty claim database records |
| `/sentiment-feed/events` | Aggregated customer feedback signals |

**Seeded Anomaly:** Handset Pro 15 Max units from lot K9R exhibit camera module failures (CAM-117) with anomalous out-of-warranty approval rates and negative sentiment spike — pointing to a manufacturing defect warranting a Field Action.

---

## API Endpoints

Once running, visit `http://localhost:8005/docs` for the interactive Swagger UI.
...
```bash
sim-generate-uc5 --output ./seed-data/
```
...
```bash
docker build \
  -t <ecr-repo>/techcompany-sim-uc5:latest \
  -f uc5-hardware-failure/techcompany-sim/Dockerfile \
  .
docker push <ecr-repo>/techcompany-sim-uc5:latest
```
...
```
http://techcompany-sim-uc5:8005/<source>/events
```
...
```bash
sim-uc5 --seed 1337 --anomaly-delay 60
```
